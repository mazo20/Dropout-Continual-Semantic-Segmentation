...
class ExtRandomScale(object):
    ...
    def __call__(self, img, lbl, sal):
        ...
        return F.resize(img, target_size, self.interpolation), \
                F.resize(lbl, target_size, F.InterpolationMode.NEAREST), \
                F.resize(sal, target_size, F.InterpolationMode.NEAREST)
...
class ExtRandomRotation(object):
    ...
    Args:
        ...
        resample ({PIL.F.InterpolationMode.NEAREST, PIL.F.InterpolationMode.BILINEAR, PIL.Image.BICUBIC}, optional):
            An optional resampling filter.
            See http://pillow.readthedocs.io/en/3.4.x/handbook/concepts.html#filters
            If omitted, or if the image has mode "1" or "P", it is set to PIL.F.InterpolationMode.NEAREST.
...
class ExtResize(object):
    ...
    def __call__(self, img, lbl, sal):
        ...
        return F.resize(img, self.size, self.interpolation), \
                F.resize(lbl, self.size, F.InterpolationMode.NEAREST), \
                F.resize(sal, self.size, F.InterpolationMode.NEAREST)
